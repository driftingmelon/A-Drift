...
function draw() {
  ...
  if (poses.length > 0) {
    ...
    // Check if the left wrist (keypoint 0) is detected
    let nose = pose.keypoints[0];
    let x = nose.x;
    let y = nose.y;
    
    if (nose && nose.score > 0.5)  { // Ensure the keypoint is detected with a confidence score
